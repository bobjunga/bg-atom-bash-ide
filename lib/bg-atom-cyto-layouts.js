...
console.log(require('util').inspect({node:node.id(), dim}, { depth: null }));
...
class NodePlacement {
	...
	console.log(require('util').inspect({l:'np',node:this.node.id(), dim:this.dim}, { depth: null }));
...
export class BGCompoundGridLayout {
	constructor( options ) {
		...
		if (this.parentNode) {
			this.directChildren = this.parentNode.children().nodes();
			this.allDescendants = this.parentNode.or(this.parentNode.descendants().nodes());
		} else {
			this.directChildren = options.eles.nodes(':orphan');
			this.allDescendants = options.eles;
		}


		//console.log("BGCompoundGridLayout on '"+this.parentNode.id()+"'("+this.parentNode.data('nodeType')+")")
		...
		this.nodePlacements = [];
		this.directChildren.forEach((node, i) => {
...
export class RecursiveCompoundLayout {
	constructor(options) {
		this.timer = new BGTimer();
		this.options = options;
		this.cy = options.cy;
		this.eles = options.eles;

		// this layout can be invoked on a compound node or on a collection
		if (!('size' in this.eles))
			this.topNode = this.eles;
		if (this.eles.size() == 1)
			this.topNode = this.eles.first();
		else
			this.topNode = null;

		if (this.topNode) {
			this.compoundNodesCollection = this.topNode.or(this.topNode.descendants(':parent'));
			console.log("RecursiveCompoundLayout called with a top node = "+this.topNode.id()+"  with "+this.compoundNodesCollection.size()+" compund nodes descendants");
		} else {
			this.compoundNodesCollection = this.eles.nodes(':parent');
			console.log("RecursiveCompoundLayout called with a collection size="+this.eles.size()+"  with "+this.compoundNodesCollection.size()+" compund nodes");
		}
		...
		// cy.extension is undocumented. cant remember why I added this line -- test removig it sometime.
		this.cy.extension('layout', 'RecursiveCompoundLayout')
		...
		this.nodeSoFar = null;
		this.curDepth = -1;
		this.compoundNodesInd = 0;
		this.compoundNodes = [];
		this.compoundNodesCollection.forEach((node) => {
			this.compoundNodes.push(nw=new CompoundNodeWrapper(node, this.topNode));
			this.curDepth = Math.max(this.curDepth, nw.depth);
		})
		this.compoundNodes.sort((a,b) => {if (""+a < ""+b) return 1; return -1;});
	}
...
export let register = function(cytoscape) {
	if (cytoscape) {
		...
		cytoscape( 'layout', 'RecursiveCompoundLayout', RecursiveCompoundLayout );
	}
};
