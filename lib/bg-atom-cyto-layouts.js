...
class VirtualGraphPositions {
	...
	sessionStart() {
		if (this.nestCount == 0) {
			this.nodeData = {};
			console.log("resetting VirtualGraphPositions on enter");
		}
		this.nestCount++;
	}
	sessionEnd() {
		...
		if (this.nestCount == 0) {
			...
			this.nodeData = {};
			console.log("resetting VirtualGraphPositions on exit");
		}
	}
...
export class RecursiveCompoundLayout {
	constructor(options) {
		...
		this.compoundNodes = [];
		this.topNode.descendants(':parent').forEach((node) => {
	...
	oneTick() {
		...
		// now finish up with the final layout
