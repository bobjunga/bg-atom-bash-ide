...
class NodePlacement {
	constructor(node) {
		this.node = node;
		this.isPlaced = false;
		this.dim = node.layoutDimensions({});
...
class LayoutGrid {
	constructor(parentNode, nodePlacements, targetGridSize) {
		...
		console.log("!!! LayoutGrid started");
	...
	apply(positions) {
		//console.log("applying layout to positions hash map")
		for (var i=0; i<this.rows.length; i++) {
			for (var j=0; j<this.rows[i].cols.length; j++) {
				var nodePlacement = this.rows[i].cols[j];

				var posNowRel = nodePlacement.node.relativePosition();
				var deltaVector = {
					x: nodePlacement.newPos.x - posNowRel.x,
					y: nodePlacement.newPos.y - posNowRel.y
				};

				//console.log("   |["+nodePlacement.node.id()+"]=("+nodePlacement.newPos.x+","+nodePlacement.newPos.y+") orig=("+posNowRel.x+","+posNowRel.y+")")
				positions[nodePlacement.node.id()] = {
					x: deltaVector.x,
					y: deltaVector.y
				};

				// if node is compound, calculate the offset vector and move (record positions for) all the descendants by the 
				// deltaVector offset
...
export class BGCompoundGridLayout {
	...
	run() {
		...
		var positions = {};
		grid.apply(positions);

		let getNodePos = ( node, i ) => {
			var curPos = node.position();
			if (node.id() in positions) {
				var offset = positions[node.id()];
				var newPos = {
					x:curPos.x + offset.x,
					y:curPos.y + offset.y
				};
				//console.log("     |CB:c="+util.inspect(curPos)+" o="+util.inspect(offset)+" n="+util.inspect(newPos));
				return newPos;
			} else {
				console.log("BGCompoundGridLayout: asked to position a node that we have not seen. node.id="+node.id()+"  the parent node we did the children for is '"+this.parentNode.id()+"'");
				return curPos;
			}
		};

		this.parentNode.descendants().layoutPositions( this, this.options, getNodePos );
	}
}
...
class CompoundNodeWrapper {
	constructor(node, layOutMaster) {
		...
		console.log("CompoundNodeWrapper:ctor id="+node.id()+"   depth="+this.depth);
		this.lay = null;
	}

	// start the layout with a stop: callback unique to this object that invokes the onLayoutStopped when its finished
	startLayout(layoutConfig) {
		this.state = "running";
		this.layOutMaster.startedCount++;
		this.layOutMaster.runningCount++;

		this.layoutConfig = Object.assign({}, layoutConfig);
		this.layoutConfig.stop = (e) => {
			console.log("CompoundNodeWrapper:stop callback for "+this.node.id());
			console.log("CompoundNodeWrapper:stop callback for "+util.inspect(e));
			this.onLayoutStopped();
		}
		this.layoutConfig.ready = () => {
			console.log("CompoundNodeWrapper:ready callback for "+this.node.id());
		}
		this.lay = this.node.layout(this.layoutConfig);
		this.lay.run();
		console.log("CompoundNodeWrapper:started layout for "+this.node.id());
	}

	// invoked asynchronously when the layout finishes
	onLayoutStopped() {
		console.log("CompoundNodeWrapper:layout ended for "+this.node.id());
		this.state = 'done';
		this.layOutMaster.runningCount--;
		this.layOutMaster.triggerMore();
	}
}
...
export class RecursiveCompoundLayout {
	constructor(topNode, onFinished) {
		...
		// kick things off by starting the topNode. We will have to run the topNode again after all its descendants have their
		// new sizes but its good visually to spread things out at first 
		this.triggerMore();
	}

	// this is called once in the ctor to start things off and then again in the stop: callback of each layout as it finishes
	triggerMore() {
		while (this.runningCount < this.maxRunning) {
			var nodeWrapper = this.getNextToRun();
			//console.log("   |> triggerMore: "+this.runningCount+" < "+this.maxRunning+"   found="+((nodeWrapper)?nodeWrapper.node.id():"null"));
			if (!nodeWrapper)
				break;
			nodeWrapper.startLayout(this.layoutConfig);
		}

		var allDone = true;
		for (nodeID in this.nodesToLayout) {
			if (this.nodesToLayout[nodeID].state != "done")
				allDone = false;
		}
	...
	// this is call when triggerMore gets called when all the nodes in nodesToLayout are in the 'done' state
	onFinish() {
		console.log("finishing");

		// must delete this because the elements contain references back to us
	...
	// This will return a nodeWrapper whose state is "waiting" or 'undefined' if there are no waiting nodes.
	// The topNode is always returned first, followed by the deepest nodes, then the next deepest, etc...
	getNextToRun() {
		if (this.nodesToLayout[this.topNode.id()].state == "waiting" || (this.startedCount % 10) == 0)
			return this.nodesToLayout[this.topNode.id()];
		var found = undefined;
		while (!found && this.curDepth > 0) {
			for (nodeID in this.nodesToLayout) {
				if (!found && this.nodesToLayout[nodeID].depth == this.curDepth && this.nodesToLayout[nodeID].state == "waiting") {
					found = this.nodesToLayout[nodeID];
				}
//console.log("!!! checking "+nodeID+"  -- "+((found)?"yes":"no"));
			};
			if (!found)
				this.curDepth--;
		}
