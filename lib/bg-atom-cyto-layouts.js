...
class CompoundNodeWrapper {
	...
	startLayout(layoutConfig) {
		...
		this.layoutConfig.stop = (e) => {
			console.log("\n>>>> CNW:stop callback for "+this.node.id());
			this.onLayoutStopped();
			console.log("<<<< CNW:stop yielding for "+this.node.id()+"\n");
		}
		this.layoutConfig.ready = () => {
			// console.log("\n>>>> CNW:ready callback for "+this.node.id());
			// console.log("<<<< CNW:ready yielding\n");
		}
		console.log("      >CNW:before layout() for "+this.node.id());
		this.lay = this.node.layout(this.layoutConfig);
		console.log("      |CNW:before run for "+this.node.id());
		this.lay.run();
		console.log("      <CNW:after run for "+this.node.id());
	}
...
export class RecursiveCompoundLayout {
	constructor(topNode, onFinished) {
		console.log("\n<<<< main ctor enter for "+topNode.id()+"\n\n");
	...
	triggerMore() {
		while (this.runningCount < this.maxRunning) {
			...
			if (!nodeWrapper)
				break;
			console.log("   TRG: starting "+nodeWrapper.node.id())
	...
	onFinish() {
		...
		this.layoutConfig.stop = () => {
			console.log("\n>>>> finishing:stop callback");
			console.log("finished");
			if (this.onFinished)
				this.onFinished();
			console.log("<<<< finishing:stop callback yielding\n");
		}
		console.log(" >starting last topNode layout");
		this.topNode.layout(this.layoutConfig).run();
		console.log(" <after run for last topNode layout");
		vGraphPositions.sessionEnd();
	}
