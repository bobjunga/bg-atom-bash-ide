...
export class BGSandboxConsole extends BGBashView {
	...
	// finish construction after our el has been added to the DOM. onConnected is called by base class
	constructAfterConnected() {
		super.constructAfterConnected()

		// if the atom window has a bg-sandbox open, initialize the sandbox features
		if (this.sbPath && this.term) {
			...
			if (fs.existsSync(path.join(this.sbPath, 'bg-core/bg-debugCntr'))) {
				this.exec('cd '+this.sbPath+'; source bg-core/bg-debugCntr vinstall .');
