...
import { newBGCompoundGridLayout as BGCompoundGridLayout, newRecursiveCompoundLayout as RecursiveCompoundLayout } from './bg-atom-cyto-layouts';
...
export class AtomCytoView extends BGAtomView {
	constructor(uri,plugin) {
		...
		this.mount([
			new bgdom.Panel({
				...
				//	new bgdom.LayoutSelector(                             (filename) => {this.applyLayout(filename)}),
					...
					new bgdom.Component('$br'),
					new bgdom.Component('$label',  "Toggle Visibility"),
					new CytoVisibilityToggle(  "btnTogImports:Imports",           this, '[edgeType *= "import"]'),
					new CytoVisibilityToggle(  "btnTogEXCMDs:Cmd calls",          this, '[edgeType *= "EXCMD"]'),
					new CytoVisibilityToggle(  "btnTogProj:Projects",             this, '[nodeType *= "project"]'),
					new CytoVisibilityToggle(  "btnTogFunc:Functions",            this, '[nodeType *= "func"]'),
					new CytoVisibilityToggle(  "btnTogAlias:Alias Fn",            this, '[subType  *= "alias"]'),
					//add to visibility? FncCallsFnc,GlbCallsFnc,FncUsesVar,FileUsesVar,CompositeFileToFile
					new bgdom.Component('$br'),
					new bgdom.Component('$label', {for: 'nodeClickToolBar'}, "Click Tools"),
		...
		//cytoscape.use( newBGCompoundGridLayout );
		//cytoscape.use( newRecursiveCompoundLayout );
	...
	// cytoscape can not be initialized before the DIV that it uses is connected in the DOM and therefore has a non-zero size.
	// constructAfterConnected is called the first time that this view is added to the document DOM tree.
	constructAfterConnected() {
		console.log('!!!AtomCytoViewconstructAfterConnected');
		try {
			this.cy = cytoscape({container : this.cytoDiv.el});
		} catch(e) {
			console.error(e);
			this.showError(e);
		}

		this.restoreDataFromFile();
	}
	...
	onToolChanged(selectedTool) {
		...
		this.cntrPanel.nodeClickToolDetails.replaceChildren([]);
	...
	onNodeClick(e) {
		switch(this.cntrPanel.nodeClickToolBar.value) {
			...
			case "nodeDetails":
				...
				this.cntrPanel.nodeClickToolDetails.replaceChildren([
					new bgdom.Component("$div Name: "+e.target.id()),
					new bgdom.Component("$div Type: "+e.target.data('nodeType')),
					new bgdom.Component("$div Classes: "+e.target.classes()),
					new bgdom.Component("$div "+ sprintf("position: (%d,%.0f) rel(%.0f,%.0f) size(%.0f,%.0f)", absPos.x,absPos.y, relPos.x,relPos.y, dim.w,dim.h)),
				])
				this.cy.resize();
				//this.cy.mount(this.cy.container());
				// mount(this.cntrPanel.nodeClickToolDetails,
				// 	new bgdom.Component("$div abs pos: ("+absPos.x+","+absPos.y+")")
				// );
				// 	new bgdom.Component("$div rel pos: ("+relPos.x+","+relPos.y+")"),
				// 	new bgdom.Component("$div dim: ("+dim.w+","+dim.h+")"),
				// ])
				break;
			default:
		}
		...
		fs.readFile(this.uri, (err, fileContents) => {
			...
			try {
				...
				this.cy.style().fromJson([
					...
					{
						"selector": "node[nodeType = 'fileGroup']",
						"style": {
							'label': 'data(label)',
							'font-size': '12pt'
						}
					...
					{
						"selector": "node[nodeType = 'externalCmd']",
						"style": {
							'width': '0.5em',
							'height': '0.5em'
						}
					...
					{
						"selector": "edge[edgeType = 'ondemandStub']",
						"style": {
							'line-style': 'dashed',
							'line-color': 'blue'
						}
