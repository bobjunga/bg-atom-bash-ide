	...
	constructor(props) {
		...
		if (this.props.URI) {
			...
			if (this.props.URI.lastIndexOf('.withPos.') == -1)
				this.saveFilename = this.props.URI.substr(0, this.props.URI.lastIndexOf('.')) + '.withPos.bgDeps';
		}
		...
		this.cntrPanel = new bgui.Panel();
		this.cntrPanel.add(new bgui.Button(                "Edit Text",       () => {this.onOpenInTextEditor()}));
		this.cntrPanel.add(new bgui.Button(                "Re-run Layout",   () => {this.runLayout()}));
		this.cntrPanel.add(new bgui.LayoutSelector(                   (filename) => {this.applyLayout(filename)}));
		this.cntrPanel.add(new bgui.Button(                "Reset",           () => {this.resetGraph()}));
		this.cntrPanel.add(new bgui.Button(                "Edges On",        () => {this.showElements(this.cy.edges())}));
		this.cntrPanel.add(new bgui.Button(                "Edges Off",       () => {this.hideElements(this.cy.edges())}));
		this.cntrPanel.add(new bgui.VisibilityToggle(this, "Unit Tests",      ".unitTest"));
		this.cntrPanel.add(new bgui.VisibilityToggle(this, "Commands",        ".command"));
		this.cntrPanel.add(new bgui.VisibilityToggle(this, "Actual Edges",    '[edgeType *= "CallsFnc"]'));
		this.cntrPanel.add(new bgui.VisibilityToggle(this, "Composite Edges", '[edgeType *= "Composite"]'));
		//FncCallsFnc,GlbCallsFnc,FncUsesVar,FileUsesVar,CompositeFileToFile

		this.nodeClickToolBar = new bgui.MutexToolGroup(
			"nodeClickToolBar",
