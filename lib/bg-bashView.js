...
export class BGBashView extends BGAtomView {
	...
	// finish construction after our el has been added to the DOM. onConnected is called when a BGComponent is connected to the
	constructAfterConnected() {
		// Create a terminal element
		this.mount([new bgdom.Component("termPanel:$div.bg-terminal")]);

		// Create the terminal
		this.term = new Terminal({convertEol: true});
console.log(this.term, this.termPanel);
global.that=this
		// attach the the term to the DOM and size
		try {
